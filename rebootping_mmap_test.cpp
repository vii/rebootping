...
define_flat_record(all_numbers_records, (int8_t, i8), (int16_t, i16), (int32_t, i32), (int64_t, i64), (uint8_t, u8), (uint16_t, u16), (uint32_t, u32),
                   (uint64_t, u64), (float, f), (double, d), );

define_flat_record(just_one_byte_records, (uint8_t, u8), );

define_flat_record(strings_records, (std::string_view, s0), (std::string_view, s1), );

define_flat_record(all_kinds_records, (int8_t, i8), (int16_t, i16), (int32_t, i32), (int64_t, i64), (uint8_t, u8), (uint16_t, u16), (uint32_t, u32),
                   (uint64_t, u64), (float, f), (double, d), (std::string_view, s), );
...
void test_flat_read_write(
    ...
    std::cout << "test_flat_read_write " << test_name << " checked " << row_num << "/" << row_count << " rows" << std::endl;
    for (auto &[k, v] : correct_field) {
        if (v != row_count) {
            std::cout << "test_flat_read_write " << test_name << " field " << k << " correct " << v << "/" << row_count << " rows" << std::endl;
        }
    }
...
TEST(flat_records, all_values) { test_flat_read_write<all_kinds_records>("all_values", row_generator_all_kinds_records, 7); }
